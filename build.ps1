...
try {
    ...
    $setup = Get-Item (Join-Path $root 'installer\out\CloudLight-Blizzard-2.1.2-win-x64-Setup.exe') -ErrorAction SilentlyContinue
    ...
    if ($setupProductVersionRaw -ne '2.1.2.0' -or $setupFileVersionRaw -ne '2.1.2.0') {
    ...
    Write-Host "Installer FileVersion: $setupFileVersion (raw $setupFileVersionRaw)"
