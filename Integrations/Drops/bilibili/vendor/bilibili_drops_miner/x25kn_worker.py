...
class X25KnWorker:
    ...
    def _notify_state(self, state: str, detail: str = "") -> None:
        ...
        try:
            ...
            LOGGER.debug("session state callback failed", exc_info=True)
    ...
    async def run_forever(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                self._notify_state("retrying", str(exc).strip() or type(exc).__name__)
                self._log_warning(
                    "直播间 %s x25Kn 运行异常: %s",
                    self.room_id,
                    exc,
    ...
    async def _trace_heartbeat_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                self._notify_state("retrying", detail)
            try:
