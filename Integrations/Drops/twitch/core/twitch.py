...
class Twitch:
    def __init__(self, settings: Settings):
        ...
        # TDM_CLAIM_TOGGLE_PATCH_BEGIN: claim-policy-state
    ...
    # TDM_CLAIM_TOGGLE_PATCH_BEGIN: claim-policy
    def is_drop_progression_blocked(self, drop_id: str) -> bool:
        return drop_id in self._claim_blocked_drops

    def clear_claim_progression_blocks(self) -> None:
        self._claim_blocked_drops.clear()
        self._reported_progression_warnings.clear()

    def _report_pending_claim(self, drop: TimedDrop) -> None:
    ...
    def _report_prerequisite_block(self, drop: TimedDrop) -> None:
        if drop.id in self._reported_progression_warnings:
            return
        self._reported_progression_warnings.add(drop.id)
        message = _("status", "prerequisite_warning").format(drop=drop.name)
        logger.warning(message)
        self.print(message)

    async def claim_drop_if_enabled(self, drop: TimedDrop) -> bool:
                ...
                for campaign in self.inventory:
                    if not campaign.upcoming:
                        for drop in campaign.drops:
                            if drop.can_claim:
                                await self.claim_drop_if_enabled(drop)
            ...
            if self.gui.progress.minute_almost_done():
                ...
                if drop_data is not None:
                    ...
                    if gql_drop is not None and gql_drop.can_earn(channel):
                        # TDM_CLAIM_TOGGLE_PATCH_BEGIN: server-progress-stall-policy
                        reported_minutes = drop_data["currentMinutesWatched"]
                        if (
                            not self.settings.auto_claim_drops
                            and gql_drop.has_unclaimed_completed_precondition
                            and reported_minutes <= gql_drop.real_current_minutes
                        ):
                            self._claim_blocked_drops.add(gql_drop.id)
                            self._report_prerequisite_block(gql_drop)
                            self.stop_watching()
                            self.change_state(State.INVENTORY_FETCH)
                            continue
                        # TDM_CLAIM_TOGGLE_PATCH_END: server-progress-stall-policy
                ...
                if not handled:
                    if (active_campaign := self.get_active_campaign(channel)) is not None:
                        # TDM_CLAIM_TOGGLE_PATCH_BEGIN: prerequisite-stall-policy
                        blocked_drop = active_campaign.first_drop
                        if (
                            not self.settings.auto_claim_drops
                            and blocked_drop is not None
                            and blocked_drop.has_unclaimed_completed_precondition
                        ):
                            self._claim_blocked_drops.add(blocked_drop.id)
                            self._report_prerequisite_block(blocked_drop)
                            self.stop_watching()
                            self.change_state(State.INVENTORY_FETCH)
                            continue
                        # TDM_CLAIM_TOGGLE_PATCH_END: prerequisite-stall-policy
    ...
    async def fetch_inventory(self) -> None:
        ...
        # TDM_CLAIM_TOGGLE_PATCH_BEGIN: refresh-claim-policy-state
        if self.settings.auto_claim_drops:
            self.clear_claim_progression_blocks()
        else:
            self._claim_blocked_drops.intersection_update(
                drop.id
