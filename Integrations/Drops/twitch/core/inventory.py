...
class BaseDrop:
    ...
    # TDM_CLAIM_TOGGLE_PATCH_BEGIN: local-preconditions
    @property
    def preconditions_satisfied_for_mining(self) -> bool:
        return all(
            prerequisite.is_claimed
            or (
                not self._twitch.settings.auto_claim_drops
                and prerequisite.watch_requirement_completed
            )
            for prerequisite_id in self.precondition_drops
            for prerequisite in (self.campaign.timed_drops[prerequisite_id],)
        )
    # TDM_CLAIM_TOGGLE_PATCH_END: local-preconditions
    ...
    def _base_earn_conditions(self) -> bool:
        ...
        return (
            self.preconditions_satisfied_for_mining  # preconditions are met
...
class TimedDrop(BaseDrop):
    ...
    # TDM_CLAIM_TOGGLE_PATCH_BEGIN: drop-progress-semantics
    @property
    def watch_requirement_completed(self) -> bool:
        return self.real_current_minutes >= self.required_minutes or self.can_claim

    @property
    def pending_claim(self) -> bool:
        return not self.is_claimed and self.watch_requirement_completed and self.can_claim

    @property
    def has_unclaimed_completed_precondition(self) -> bool:
        return not self._twitch.settings.auto_claim_drops and any(
            not prerequisite.is_claimed and prerequisite.watch_requirement_completed
            for prerequisite_id in self.precondition_drops
            for prerequisite in (self.campaign.timed_drops[prerequisite_id],)
        )

    @property
    def eligible_for_local_progression(self) -> bool:
        blocked_check = getattr(self._twitch, "is_drop_progression_blocked", None)
        return (
            (self._twitch.settings.auto_claim_drops or not self.watch_requirement_completed)
            and (blocked_check is None or not blocked_check(self.id))
        )
    # TDM_CLAIM_TOGGLE_PATCH_END: drop-progress-semantics
    ...
    def _base_earn_conditions(self) -> bool:
        return (
            ...
            and self.eligible_for_local_progression
    ...
    def _bump_minutes(self, channel: Channel | None) -> bool:
        # TDM_CLAIM_TOGGLE_PATCH_BEGIN: stop-fake-progress
        if self.has_unclaimed_completed_precondition:
            return False
        # TDM_CLAIM_TOGGLE_PATCH_END: stop-fake-progress
