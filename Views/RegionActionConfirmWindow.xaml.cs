    ...
    public RegionActionConfirmWindow(RegionActionConfirmKind kind, OverwatchRegion? region = null)
    {
        ...
        switch (kind)
        {
            case RegionActionConfirmKind.Prepare:
                ...
                FooterText.Text = "接下来软件会先保存当前完整游戏文件到本地临时区域。这个过程只会读取和复制本地文件，不会产生网络流量。";
